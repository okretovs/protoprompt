...
The Day 0 MVP requires a user-provided OpenAI key before idea submission, but that key is an in-memory session credential only. After submission, the UI hides the masked value to reduce accidental exposure during demos. It is not persisted to a database, localStorage, sessionStorage, cookies, or docs.
...
- Missing server `OPENAI_API_KEY` should not block a Day 0 demo if the user supplies an in-memory key.
