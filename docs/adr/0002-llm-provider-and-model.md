...
- Day 0 requires a user-provided OpenAI API key before idea submission as an in-memory run credential, while still supporting the server-side `OPENAI_API_KEY`. This keeps the complete 7-stage flow demoable when server configuration is missing.
...
- User-provided OpenAI keys are transient run inputs, not account data. They must not be persisted, logged, committed, or written to browser storage.
